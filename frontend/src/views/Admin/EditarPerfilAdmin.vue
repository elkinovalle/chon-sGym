<template>
  <v-form
    ...
    <v-container class="color" fluid>
      <v-layout row>
        <v-flex xs5 order-lg2>
         ...
         <v-flex xs8 order-lg2>
            <v-card-text  class="transparent text-xs-center">
            <h1 class="text-xs-  headline"><strong>DATOS DE PERFIL</strong></h1><br>
             </v-card-text>
            <v-layout  row>
      ...
      <v-layout  row>
        <v-flex   xs2 order-lg2 class="transparent text-xs-center"></v-flex>
        <v-flex xs3 order-lg2>
             <v-dialog v-model="dialogEmail" persistent max-width="600px">
          <template v-slot:activator="{ on }">
            <v-btn block dark v-on="on" class="boton" flat>Editar e-mail</v-btn>
          </template>
        ...
        <template v-slot:activator="{ on }">
        <v-btn block dark v-on="on" class="boton" flat>Editar Contraseña</v-btn>
        </template>
        <v-card class="contenido">
          <v-card-title>
